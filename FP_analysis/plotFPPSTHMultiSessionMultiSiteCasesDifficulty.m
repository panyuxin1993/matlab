...
%         trialType_cat_animal=fR
        ...
        if size(celldata_by_datapoint,2)==6 && strcmp(AUCstr{iAUCstr},'hardest-easiest')
            celldata_by_datapoint=celldata_by_datapoint(:,[1,3,4,6],:);
            celldata_by_site=celldata_by_site(:,[1,3,4,6],:);
        end
...
%             figDiffMeanTrace=figure;
            ...
            for iResult=1%:length(resultStr) 
                ...
                for iData=1:size(celldata_by_datapoint,3)%for each session
                    ...
                    StructDiff(iData).earlyData = nanmean(Structout.data(:,(frameNumTime(1))*1000/frT:(frameNumTime(1)+0.9)*1000/frT),2);
                    StructDiff(iData).lateData = nanmean(Structout.data(:,(frameNumTime(1)+0.9)*1000/frT:(frameNumTime(1)+1.5)*1000/frT),2);
                    StructDiff(iData).stimData = nanmean(Structout.data(:,(frameNumTime(1)-0.5)*1000/frT:(frameNumTime(1))*1000/frT),2);
                    StructDiff(iData).delayData = nanmean(Structout.data(:,(frameNumTime(1))*1000/frT:(frameNumTime(1)+1.5)*1000/frT),2);%whole delay period         
                    ...
                    if ~exist('choiceAUC_diffdiff','var') 
                        ...
                        for i=1:n_difficulty
                            ...
                            [choiceAUC_diffdiff(i,:),pchoiceAUC_diffdiff(i,:)] = fMovingAUC(tempStruct.label,tempStruct.data,poslabel,nshuffle,binsize,binstep);
                        end
                    end
                    [StructDiff(iData).choiceAUC_diffdiff,StructDiff(iData).pchoiceAUC_diffdiff] = deal(choiceAUC_diffdiff,pchoiceAUC_diffdiff);
                    if length(unique(StructDiff(iData).label))==2
                        if ~exist('difficultyAUCcase','var')
                            [difficultyAUCcase,difficultypAUCcase] = fMovingAUC(StructDiff(iData).label,StructDiff(iData).data,poslabel,nshuffle,binsize,binstep);
                        end
                        [StructDiff(iData).AUC,StructDiff(iData).pAUC] = deal(difficultyAUCcase,difficultypAUCcase);
                        ...
                        if ~exist('AUCstim','var')
                            [AUCstim,pAUCstim]=fAUC(StructDiff(iData).label,StructDiff(iData).stimData,poslabel,nshuffle);
                            [AUCdelay,pAUCdelay] = fAUC(StructDiff(iData).label,StructDiff(iData).delayData,poslabel,nshuffle);  
                        end
                        [StructDiff(iData).AUCearly,StructDiff(iData).pAUCearly,StructDiff(iData).AUClate,StructDiff(iData).pAUClate,...
                            StructDiff(iData).AUCstim,StructDiff(iData).pAUCstim,StructDiff(iData).AUCdelay,StructDiff(iData).pAUCdelay] = ...
                            deal(AUCearly,pAUCearly,AUClate,pAUClate,AUCstim,pAUCstim,AUCdelay,pAUCdelay);
                         save(fileNameDifficultyAUC_datapoint,'difficultyAUCcase','difficultypAUCcase','AUCearly','pAUCearly','AUClate','pAUClate','AUCstim','pAUCstim','AUCdelay','pAUCdelay','choiceAUC_diffdiff','pchoiceAUC_diffdiff');
                        clearvars  'AUCearly' 'pAUCearly' 'AUClate' 'pAUClate' 'AUCstim' 'pAUCstim' 'AUCdelay' 'pAUCdelay' 'difficultyAUCcase' 'difficultypAUCcase' 'choiceAUC_diffdiff' 'pchoiceAUC_diffdiff'
                    elseif length(unique(StructDiff(iData).label))>2 %using anova
                        ...
                        StructDiff(iData).pANOVA1stim=fANOVA1(StructDiff(iData).stimData,StructDiff(iData).label);
                        StructDiff(iData).pANOVA1delay=fANOVA1(StructDiff(iData).delayData,StructDiff(iData).label);
                        save(fileNameDifficultyAUC_datapoint,'choiceAUC_diffdiff','pchoiceAUC_diffdiff');
                        clearvars 'choiceAUC_diffdiff' 'pchoiceAUC_diffdiff'
                    else
                ...
                for i_difficulty=1:size(tempcellAUC,2)
                    for i_case= 1:size(tempcellAUC,3)
                      tempcellAUC{1,i_difficulty, i_case}= StructDiff(i_case).choiceAUC_diffdiff(i_difficulty,:);
                    end
                end
                [figAUCCases,curve_AUCcaseTrace] = fPlotCases(ts,tempcellAUC(1,:,:),col_figcase,color_diff,behEventAlign,'choice AUC',frameNumTime,selectivitystr{i_selectivity},stimDur,sigX,casename);
                %plot by datapoint
                %neuralActivity{nResult,nStim}(ncase,timepoint)
                [figCases,curve_caseTrace] = fPlotCases(ts,celldata_by_datapoint(iResult,:,:),col_figcase,color_cases_trace,behEventAlign,'\it\DeltaF/F',frameNumTime,selectivitystr{i_selectivity},stimDur,sigX,casename);
                neuralActivity_by_datapoint_diff=fContraIpsiDiff(celldata_by_datapoint(iResult,:,:),preferedStr{iPrefered});
                [figCasesDiff,curve_caseDiffTrace] = fPlotCases(ts,neuralActivity_by_datapoint_diff,col_figcase,color_diff,behEventAlign,'\it\DeltaF/F(contra-ipsi)',frameNumTime,selectivitystr{i_selectivity},stimDur,sigX,casename);
                clearvars casename
                ...
                for iData=1:size(celldata_by_site,3)%for each session
                    ...
                    StructDiffSite(iData).earlyData = nanmean(StructoutSite.data(:,(frameNumTime(1))*1000/frT:(frameNumTime(1)+0.9)*1000/frT),2);
                    StructDiffSite(iData).lateData = nanmean(StructoutSite.data(:,(frameNumTime(1)+0.9)*1000/frT:(frameNumTime(1)+1.5)*1000/frT),2);
                    StructDiffSite(iData).stimData = nanmean(StructoutSite.data(:,(frameNumTime(1)-0.5)*1000/frT:(frameNumTime(1))*1000/frT),2);
                    StructDiffSite(iData).delayData = nanmean(StructoutSite.data(:,(frameNumTime(1))*1000/frT:(frameNumTime(1)+1.5)*1000/frT),2);
                    ...
                    [StructDiffSite(iData).choiceAUC_diffdiff,StructDiffSite(iData).pchoiceAUC_diffdiff] = deal(choiceAUC_diffdiff,pchoiceAUC_diffdiff);
                    if length(unique(StructDiffSite(iData).label))==2
                        if ~exist('difficultyAUCcase','var')
                            [difficultyAUCcase,difficultypAUCcase] = fMovingAUC(StructDiffSite(iData).label,StructDiffSite(iData).data,poslabel,nshuffle,binsize,binstep);
                        end
                        [StructDiffSite(iData).AUC,StructDiffSite(iData).pAUC] = deal(difficultyAUCcase,difficultypAUCcase);
                        if ~exist('AUCearly','var')
                        ...
                        if ~exist('AUCstim','var')
                            [AUCstim,pAUCstim]=fAUC(StructDiffSite(iData).label,StructDiffSite(iData).stimData,poslabel,nshuffle);
                            [AUCdelay,pAUCdelay] = fAUC(StructDiffSite(iData).label,StructDiffSite(iData).delayData,poslabel,nshuffle);  
                        end
                        [StructDiffSite(iData).AUCearly,StructDiffSite(iData).pAUCearly,StructDiffSite(iData).AUClate,StructDiffSite(iData).pAUClate,...
                            StructDiffSite(iData).AUCstim,StructDiffSite(iData).pAUCstim,StructDiffSite(iData).AUCdelay,StructDiffSite(iData).pAUCdelay] = ...
                            deal(AUCearly,pAUCearly,AUClate,pAUClate,AUCstim,pAUCstim,AUCdelay,pAUCdelay);
                         save(fileNameDifficultyAUC_site,'difficultyAUCcase','difficultypAUCcase','AUCearly','pAUCearly','AUClate','pAUClate','AUCstim','pAUCstim','AUCdelay','pAUCdelay','choiceAUC_diffdiff','pchoiceAUC_diffdiff');
                        clearvars  'AUCearly' 'pAUCearly' 'AUClate' 'pAUClate' 'AUCstim' 'pAUCstim' 'AUCdelay' 'pAUCdelay' 'difficultyAUCcase' 'difficultypAUCcase' 'choiceAUC_diffdiff' 'pchoiceAUC_diffdiff'
                    ...
                    elseif length(unique(StructDiffSite(iData).label))>2 %using anova
                        ...
                        StructDiffSite(iData).pANOVA1stim=fANOVA1(StructDiffSite(iData).stimData,StructDiffSite(iData).label);
                        StructDiffSite(iData).pANOVA1delay=fANOVA1(StructDiffSite(iData).delayData,StructDiffSite(iData).label);
                        save(fileNameDifficultyAUC_site,'choiceAUC_diffdiff','pchoiceAUC_diffdiff');
                        clearvars 'choiceAUC_diffdiff' 'pchoiceAUC_diffdiff'
                    else
                ...
                for i_difficulty=1:size(tempcellAUC,2)
                    for i_case= 1:size(tempcellAUC,3)
                      tempcellAUC{1,i_difficulty, i_case}= StructDiffSite(i_case).choiceAUC_diffdiff(i_difficulty,:);
                    end
                end
                [figAUCCases,curve_AUCcaseTrace] = fPlotCases(ts,tempcellAUC(1,:,:),col_figcase,color_diff,behEventAlign,'choice AUC',frameNumTime,selectivitystr{i_selectivity},stimDur,sigX,casename);

                %plot by site
                [figSite,curve_siteTrace] = fPlotCases(ts,celldata_by_site(iResult,:,:),col_figsite,color_cases_trace,behEventAlign,'\it\DeltaF/F',frameNumTime,selectivitystr{i_selectivity},stimDur,sigX,casename);
                neuralActivity_by_site_diff=fContraIpsiDiff(celldata_by_site(iResult,:,:),preferedStr{iPrefered});
                [figSiteDiff,curve_siteDiffTrace] = fPlotCases(ts,neuralActivity_by_site_diff,col_figsite,color_diff,behEventAlign,'\it\DeltaF/F(contra-ipsi)',frameNumTime,selectivitystr{i_selectivity},stimDur,sigX,casename);  
                clearvars casename
...
%                 end
    ...
    clearvars celltypeVar answerVar StructDiff
    %for site
    ...
    clearvars celltypeVar answerVar StructDiffSite
end
%}
%
...
%         fPlotMean_SE( ts,matAUC,color_celltype_mean{ncelltype});
        ylim([0,1]);
        xlim([ts(1),ts(end)]);
...
%         fPlotMean_SE( ts,matAUCsite,color_celltype_mean{ncelltype});
        ylim([0,1]);
        xlim([ts(1),ts(end)]);
        ylabel('AUC distinguishing easy/hard');
        xlabel('Time (s) from delay onset');
...
if sum(tempANOVAvar)==0 % only have AUC value
    pSig=0.05;
    figEpochAUC=figure;
    set(gcf,'PaperPosition',[1,1,8,4]);
    subplot(2,4,1);
    fScatterAUCCmpOneEpoch(Tdifficulty,pSig,'stim');
    title('stim AUC by session');
    subplot(2,4,2);
    fScatterAUCCmpOneEpoch(Tdifficulty,pSig,'delay');
    title('delay AUC by session');
    subplot(2,4,5);
    fScatterAUCCmpOneEpoch(TdifficultySite,pSig,'stim');
    title('stim AUC by site');
    subplot(2,4,6);
    fScatterAUCCmpOneEpoch(TdifficultySite,pSig,'delay');
    title('delay AUC by site');
        subplot(2,4,3);
    fScatterAUCCmpOneEpoch(Tdifficulty,pSig,'early');
    title('early delay AUC by session');
    subplot(2,4,4);
    fScatterAUCCmpOneEpoch(Tdifficulty,pSig,'late');
    title('late delay AUC by session');
    subplot(2,4,7);
    fScatterAUCCmpOneEpoch(TdifficultySite,pSig,'early');
    title('early delay AUC by site');
    subplot(2,4,8);
    fScatterAUCCmpOneEpoch(TdifficultySite,pSig,'late');
    title('late delay AUC by site');
...
for nResult=1:size(trialtype,1) %4 column(correct/error/miss/violation),companied with 4 lick raster
    for nStim=1:size(trialtype,2) %for each stimulus%[1,6]%just 2 end trials
        ...
        for idatapoint=1:length(dff)
            selectedTrialInd=trialtypecell{idatapoint}(nResult,nStim,:);
...
function [figCases,curve_caseTrace] = fPlotCases(ts,neuralActivity,col_figcase,color_cases_trace,behEventAlign,ylabelstr,frameNumTime,selectivitystr,stimDur,SigX,casename)
...
%             plot(ts,neuralActivity{nResult,nStim,i_case},'color',color_cases_trace{nResult,nStim});
    ...
    if i_case==n_cases
        switch selectivitystr
            case 'choice'
                h=legend(curve_caseTrace(:),'ipsi correct','contra correct','AutoUpdate','off');
                %                             legend(curve_caseTrace(:),'ipsi correct','contra correct','ipsi error','contra error');
            case 'difficulty'
                if length(curve_caseTrace)==4
                    h=legend(curve_caseTrace(:),'ipsi easy','ipsi hard','contra hard','contra easy','AutoUpdate','off');
                elseif length(curve_caseTrace)==2
                    h=legend(curve_caseTrace(:),'easy','hard','AutoUpdate','off');
                end
            case 'stimuli'
                if length(curve_caseTrace)==6
                    h=legend(curve_caseTrace(:),'','ipsi','','','contra','','AutoUpdate','off');
                elseif length(curve_caseTrace)==4    
                    h=legend(curve_caseTrace(:),'ipsi','','','contra','AutoUpdate','off');
                elseif length(curve_caseTrace)==3
                    h=legend(curve_caseTrace(:),'easy','medium','hard','AutoUpdate','off');
                elseif length(curve_caseTrace)==2
                    h=legend(curve_caseTrace(:),'easy','hard','AutoUpdate','off');
                end
        end
    ...
    casenamestr=strrep(casename{i_case},'_','-');
    title(casenamestr);
end
end
...
for i=1:n_case
    ...
    plot([0,0],[0,1],'k-');
    plot([-0.5,-0.5],[0,1],'k-');
    casenamestr=strrep(casename{i},'_','-');
    title(casenamestr);
    ylim([0,1]);
    xlim([ts(1),ts(end)])
...
if strcmp(epoch,'early')
    ind_col=12:13;
    ylabelstr='early delay AUC';
elseif strcmp(epoch,'late')
    ind_col=14:15;
    ylabelstr='late delay AUC';
elseif strcmp(epoch,'stim')
    ind_col=16:17;
    ylabelstr='stim AUC';
elseif strcmp(epoch,'delay')
    ind_col=18:19;%*******************need to check
    ylabelstr='delay AUC';
end
...
%shift to absolute AUC value,no direction
TAUCVglut2=abs(TAUCVglut2-0.5)+0.5;
TAUCVgat=abs(TAUCVgat-0.5)+0.5;
...
[hNor1,pNor1] = chi2gof(TAUCVglut2(:,1),'Alpha',0.05);
[hNor2,pNor2] = chi2gof(TAUCVgat(:,1),'Alpha',0.05);
if hNor1==0 || hNor2==0
    p1=ranksum(TAUCVglut2(:,1),TAUCVgat(:,1));
    text(1,1,'ranksum test');
else
    [h,p1]=ttest2(TAUCVglut2(:,1),TAUCVgat(:,1));
end
pvglut2 = signrank(TAUCVglut2(:,1)-0.5);
pvgat = signrank(TAUCVgat(:,1)-0.5);

xlim([0,3]);
ylim([0,1]);
plot([1,1.9],[0.8,0.8],'k-');
text(1,0.9,plabelsymbol(p1));
% text(0.7,0.8,plabelsymbol(pvglut2));
% text(2.3,0.8,plabelsymbol(pvgat));
