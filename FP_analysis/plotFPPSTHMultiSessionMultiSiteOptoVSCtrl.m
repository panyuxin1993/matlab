...
%     ind_session=strcmp(T.animal,whichAnimal).*strcmp(T.used_as_data,'yes').*(strcmp(T.date,'2020/9/18'));
    animal_name='pyx295';
    ...
    for nfiber=1:length(fiberstr)%here means fiber bilaterally implanted,1-soma, 2-terminal
        ...
        dataInfoStr=strcat(experiment,'-',regionstr{i_region},'-',fiberstr{nfiber},'-opto ',position2opto{i_position2opto});
        dataProcessStr=strcat('-grouped by ',selectivitystr{i_selectivity},'-binCentered-size',num2str(binsize),'-shuffle',num2str(nshuffle),'-timeWindow',num2str(frameNumTime(1)),'-',num2str(frameNumTime(2)));
        fileNameAUC=strcat(savepath,filesep,dataInfoStr,dataProcessStr,'-n',num2str(n_datapoint(nfiber)),'-sites',num2str(n_site(nfiber)),'-opto.mat');
        ...
        for i=1:n_session%遍历满足条件的session并合并
            if nfiber==1%soma
                if strcmp(position2opto{i_position2opto},'ipsi')
                    if strcmp(T.brain_region(ind_session(i)),'left SC') && (contains(T.manipulation(ind_session(i)),'left')||contains(T.manipulation(ind_session(i)),'bilateral'))
                        fiberSide={'left'};
                        ind_fiber=[1];
                    elseif strcmp(T.brain_region(ind_session(i)),'right SC')&& (contains(T.manipulation(ind_session(i)),'right')||contains(T.manipulation(ind_session(i)),'bilateral'))
                        fiberSide={'right'};
                        ind_fiber=[2];
                    elseif strcmp(T.brain_region(ind_session(i)),'bilateral SC')
                        if contains(T.manipulation(ind_session(i)),'right')
                            fiberSide={'right'};
                            ind_fiber=[2];
                        elseif contains(T.manipulation(ind_session(i)),'left')
                            fiberSide={'left'};
                            ind_fiber=[1];
                        elseif contains(T.manipulation(ind_session(i)),'bilateral')
                ...
                elseif strcmp(position2opto{i_position2opto},'contra')
                    if strcmp(T.brain_region(ind_session(i)),'left SC') && contains(T.manipulation(ind_session(i)),'right')
                        fiberSide={'left'};
                        ind_fiber=[1];
                    elseif strcmp(T.brain_region(ind_session(i)),'right SC')&& contains(T.manipulation(ind_session(i)),'left')
                        fiberSide={'right'};
                        ind_fiber=[2];
                    elseif contains(T.brain_region(ind_session(i)),'bilateral SC')
                        if contains(T.manipulation(ind_session(i)),'left')
                            fiberSide={'right'};
                            ind_fiber=[2];
                        elseif contains(T.manipulation(ind_session(i)),'right')
                            fiberSide={'left'};
                            ind_fiber=[1];
                        elseif contains(T.manipulation(ind_session(i)),'bilateral')
                            fiberSide=[];
...
%         trialType_cat_animal=fR
        ...
        if size(celldata_by_datapoint,4)==2  && size(celldata_by_datapoint,2)==2
            ...
            temp_datapoint(:,2,:)=celldata_by_datapoint(:,1,:,2);
            temp_datapoint(:,3,:)=celldata_by_datapoint(:,2,:,2);
            temp_datapoint(:,4,:)=celldata_by_datapoint(:,2,:,1);
            celldata_by_datapoint=temp_datapoint;
            temp_site(:,1,:)=celldata_by_site(:,1,:,1);
            temp_site(:,2,:)=celldata_by_site(:,1,:,2);
            temp_site(:,3,:)=celldata_by_site(:,2,:,2);
            temp_site(:,4,:)=celldata_by_site(:,2,:,1);
        ...
        for ndff=[1]         %  titlestr=strcat(animal_name,'-',fiberstr{nfiber},'-',titlestr);
            ...
            for figrow=1:2
                for nResult=1:size(neuralActivity2Plot{figrow},1) %4 column(correct/error/miss/violation),companied with 4 lick raster
                    ...
                    set(gca,'xtick',[-floor(frameNumTime(1)):1:frameNumTime(2)],'xticklabel',[-floor(frameNumTime(1)):1:frameNumTime(2)]);
                    ylim(y_lim);
            ...
            set(h,'box','off');
            text(0,1,strcat('n=',num2str(n_datapoint),' datapoints',num2str(n_session),' sessions,',num2str(n_site(nfiber)),' site,',num2str(n_animal),' animal'),'FontName','Arial','FontSize',10,'Unit','Normalized');
            
            %plot cases
            if size(celldata_by_datapoint,2)==6 %strcmp(selectivitystr{i_selectivity},'stimuli')
            ...
            elseif size(celldata_by_datapoint,2)==4 %strcmp(selectivitystr{i_selectivity},'difficulty')
                color_cases_trace={[0 0 1],[0 0.5 1],[1 0.5 0],[1 0 0]};%left-low--right-high;since cat sessions together, color don't care rule, just using the standard condition
                color_diff={[0 0 0],[0.5 0.5 0.5]};%left-low--right-high;since cat sessions together, color don't care rule, just using the standard condition
            end
...
%             figDiffMeanTrace=figure;
            ...
            for iResult=1%:length(resultStr) 
                ...
                for iData=1:size(celldata_by_datapoint,3)%for each session
                    ...
                    StructDiff(iData).lateData = nanmean(Structout.data(:,(frameNumTime(1)+0.5)*1000/frT:(frameNumTime(1)+1)*1000/frT),2);
                    StructDiff(iData).datapointName=datapointNameCell{iData};
                    fileNameAUC_datapoint=strcat(savepath,filesep,dataInfoStr,dataProcessStr,'-',StructDiff(iData).datapointName,dataProcessStr,'-difficultyAUC.mat');%adding datapoint name to the AUC file name
                    ...
                    if length(unique(StructDiff(iData).label))==2
                        ...
                        [StructDiff(iData).AUCearly,StructDiff(iData).pAUCearly,StructDiff(iData).AUClate,StructDiff(iData).pAUClate] = deal(AUCearly,pAUCearly,AUClate,pAUClate);
                         save(fileNameAUC_datapoint,'optoctrlAUCcase','poptoctrlAUCcase','AUCearly','pAUCearly','AUClate','pAUClate','choiceAUC_diffopto','pchoiceAUC_diffopto');
                        clearvars  'optoctrlAUCcase' 'poptoctrlAUCcase' 'AUCearly' 'pAUCearly' 'AUClate' 'pAUClate' 'choiceAUC_diffopto' 'pchoiceAUC_diffopto'
                    elseif length(unique(StructDiff(iData).label))>2 %using anova
                        ...
                        save(fileNameAUC_datapoint,'choiceAUC_diffopto','pchoiceAUC_diffopto');
                ...
                for iData=1:size(celldata_by_site,3)%for each session
                    ...
                    StructDiffSite(iData).lateData = nanmean(StructoutSite.data(:,(frameNumTime(1)+0.5)*1000/frT:(frameNumTime(1)+1)*1000/frT),2);
                    StructDiffSite(iData).siteName=siteNameCell(iData).nameSite;
                    fileNameAUC_site=strcat(savepath,filesep,dataInfoStr,dataProcessStr,'-',StructDiffSite(iData).siteName{1},dataProcessStr,'-difficultyAUC.mat');%adding datapoint name to the AUC file name
                    ...
                    if length(unique(StructDiffSite(iData).label))==2
                        ...
                        [StructDiffSite(iData).AUCearly,StructDiffSite(iData).pAUCearly,StructDiffSite(iData).AUClate,StructDiffSite(iData).pAUClate] = deal(AUCearly,pAUCearly,AUClate,pAUClate);
                         save(fileNameAUC_site,'optoctrlAUCcase','poptoctrlAUCcase','AUCearly','pAUCearly','AUClate','pAUClate','choiceAUC_diffopto','pchoiceAUC_diffopto');
                        clearvars  'optoctrlAUCcase' 'poptoctrlAUCcase' 'AUCearly' 'pAUCearly' 'AUClate' 'pAUClate' 'choiceAUC_diffopto' 'pchoiceAUC_diffopto'
                    elseif length(unique(StructDiffSite(iData).label))>2 %using anova
                        ...
                        save(fileNameAUC_site,'choiceAUC_diffopto','pchoiceAUC_diffopto');
                        clearvars 'choiceAUC_diffopto' 'pchoiceAUC_diffopto'
                    else
                ...
                save(fileNameAUC,'StructDiff','StructDiffSite');
                ...
                [figAUCCases,curve_AUCcaseTrace] = fPlotCases(ts,tempcellAUC(1,:,:),col_figsite,color_diff,behEventAlign,'choice AUC',frameNumTime,selectivitystr{i_selectivity},stimDur,sigX);
...
%                 saveas(figSiteDiff,[savepath,filesep,experiment,'-',regionstr{i_region},'-',fiberstr{nfiber},'-grouped by ',selectivitystr{i_selectivity},'-algin to ',behEventAlign,'-masklick-',masklick,'-individual cases difference by site-',sigLabel,'.pdf'],'pdf');
    ...
    celltypeVar=categorical(celltypeVar);
    answerVar=categorical(answerVar);
    tempTsite=struct2table(StructDiffSite,'AsArray',true);
    ...
    clearvars 'celltypeVar' 'answerVar' 'StructDiff' 'StructDiffSite'
end
%}
...
for ncelltype=1:length(experiment)
    dataInfoStr=strcat(experiment{ncelltype},'-',regionstr{i_region},'-',fiberstr{nfiber},'-opto ',position2opto{i_position2opto});
    fileNameAUC=strcat(savepath,filesep,dataInfoStr,dataProcessStr,'-n',num2str(n_datapointCell{ncelltype}(1)),'-sites',num2str(n_siteCell{ncelltype}(1)),'-opto.mat');
    load(fileNameAUC);
    ...
    ylim([0,1]);
    xlim([ts(1),ts(end)]);
    ylabel('AUC distinguishing ctrl/opto');
...
saveas(figDiffAUC,[savepath,filesep,'optoMovingAUC-algin to ',behEventAlign,'-masklick-',masklick,'.pdf'],'pdf');
...
if sum(tempAUCvar)>0 % have AUC value
    ...
    saveas(figEpochAUC,[savepath,filesep,'optoEpochAUC-algin to ',behEventAlign,'-masklick-',masklick,'.pdf'],'pdf');
            ...
            switch selectivitystr
                case 'choice'
                    if length(curve_caseTrace)==2
                        h=legend(curve_caseTrace(:),'ctrl','opto','AutoUpdate','off');
                    elseif length(curve_caseTrace)==4
                        h=legend(curve_caseTrace(:),'ipsi ctrl','ipsi opto','contra opto','contra ctrl','AutoUpdate','off');
                    end
