...
for i_celltype=1:2
    ...
    dff_aligned_cat_animal=cell(2,n_site(1));%two fibers so two cells to store,for multiple sessions, store 1D--1-soma,2-terminal data,2D--sites; note soma sites are more than terminal sites
    trialType_cat_animal=cell(2,n_site(1));
    behEvent_aligned_cat_animal=cell(2,n_site(1));
    licking_aligned_cat_animal=cell(2,n_site(1));
    dff_aligned_by_session=cell(2,n_datapoint(1));%2D-data points, each means one site one session
    trialType_by_session=cell(2,n_datapoint(1));
    behEvent_aligned_by_session=cell(2,n_datapoint(1));
    licking_aligned_by_session=cell(2,n_datapoint(1));
    for nfiber=1:length(fiberstr)%here means fiber bilaterally implanted,1-soma, 2-terminal
        %store AUC and p-value
        fileNameAUCrecent=strcat(savepath,filesep,'recentAUCfile');
