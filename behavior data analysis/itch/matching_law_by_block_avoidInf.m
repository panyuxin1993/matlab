...
for i=1:nsession
    ...
    for j=1:nblock(i)
        ...
        matching_law_data(jblock,1)=(100-pleft);
        matching_law_data(jblock,2)=sum(choice(blockInd(j):blockInd(j+1)-1)==1)/sum(choice(blockInd(j):blockInd(j+1)-1)<2);
        matching_law_data(jblock,3)=sum(rightLick(blockInd(j):blockInd(j+1)-1))/(sum(leftLick(blockInd(j):blockInd(j+1)-1))+sum(rightLick(blockInd(j):blockInd(j+1)-1)));
    end
end
%fitting
...
for i=1:2
    ...
    xlabel('Ratio of right side reward probability(log2)');
    ylabel(strcat('Ratio of right side',ystr{i},' probability(log2)'));
