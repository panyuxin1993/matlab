%分析不同delay长度的正确率，miss,violation比例,reaction time
clear;
ndelaygroup=4;
date='2020_01_11';
animal='pyx239';
if contains(animal,'pyx17') 
    date(date=='_')='';%将下划线取消掉
    filename=strcat('D:\xulab\behavior\',animal,'\',animal,'_',date,'_2AFC.mat'); 
elseif contains(animal,'pyx1') ||contains(animal,'pyx273') ||contains(animal,'pyx26')||contains(animal,'pyx23')||contains(animal,'pyx241')
    filename=strcat('D:\xulab\behavior\',animal,'\',date,'_',animal,'.mat');
elseif contains(animal,'pyx231')
    date(date=='_')='';%将下划线取消掉
    filename=strcat('D:\xulab\behavior\',animal,'\','PYX231',32,date,'.mat'); %here 32 means ' ' 
else %imaging
    date(date=='_')='';%将下划线取消掉
    filename=strcat('D:\xulab\behavior\',animal,'\',animal,'_',date,'.mat'); 
end 
