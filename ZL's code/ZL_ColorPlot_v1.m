...
[ CorrTrial,WroTrial,MisTrial] = TrialType_fun(Data_extract,TestTrialNum,1);
...
Fre = unique(Data_extract.Tone_frequency)/1000;
Lick_out = FirstLickAndRewTime_fun(Data_extract,TestTrialNum);
...
RewardTime = round(Lick_out.RewardT(3,:)/FrameTime) + double(MinOnsFrame);
for nROI = 1:size(SeleData,2)
   ...
   for i = 1:size(CorrTrial,1)
       ...
       line([SeleOnsFrame SeleOnsFrame]+round(300/FrameTime),[0.5 sum(CorrTrialInd)+0.5],'linestyle','--','linewidth',LineW*2,'color','w');
       line([SeleOnsFrame SeleOnsFrame]+round(800/FrameTime),[0.5 sum(CorrTrialInd)+0.5],'linestyle','-','linewidth',LineW*3,'color',[.7 .7 .7]);       
...
%        xlim([0 size(ROI_data,2)]);
       ...
       line([SeleOnsFrame SeleOnsFrame]+round(300/FrameTime),[0.5 sum(WroTrialInd)+0.5],'linestyle','--','linewidth',LineW*2,'color','w');
       line([SeleOnsFrame SeleOnsFrame]+round(800/FrameTime),[0.5 sum(WroTrialInd)+0.5],'linestyle','-','linewidth',LineW*3,'color',[.7 .7 .7]);           
