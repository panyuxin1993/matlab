function [outputArg1,outputArg2] = fCorrelationBehaviorDistributedSession(filepath, bodyparts,coordinates,pSigTtest,treshold4likelihood,behEventAlign,saveNameID,yrange,titlestr,nonOverlappingBin )
%FCORRELATIONBEHAVIORDISTRIBUTEDSESSION plot coordinates shift or likelihood for
%multiple session multiple body parts, all video files are stored in
%distributed folders, with summary file indicating their paths
...
savepath=[filepath,filesep,'summary',filesep,'video_tracking'];
...
for iSession=1:nSession%can be a loop
    led_file=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,dataSummaryT.OLEDFileName{iSession},'.csv'];
    behdata=[dataSummaryT.rootpath{iSession},filesep,dataSummaryT.session{iSession},'_Virables.mat'];
    video_file=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,dataSummaryT.videoName{iSession},'-crop-mjpeg.avi'];%raw video seems have some problems in frame rate
    v_reader=VideoReader(video_file);
    video_frameRate=v_reader.FrameRate;
    fr=max(video_frameRate/nonOverlappingBin,fr);%use max frame rate for common use
    ...
    if ~exist([dataSummaryT.rootpath{iSession},filesep,'video',filesep,name_file_led2{1},'.mat'],'file')
        T=readtable(led_file);%different sessions may need mannually set parameters and save result separately
        frameTrialStartVideo=fTrialStartByOLEDrefArduino( T, video_frameRate ,timeTrialStartBeh);
        save([dataSummaryT.rootpath{iSession},filesep,'video',filesep,name_file_led2{1},'.mat'],'frameTrialStartVideo');
    else
        load([dataSummaryT.rootpath{iSession},filesep,'video',filesep,name_file_led2{1},'.mat'])
    end
    ...
    for iBodyPart=1:nbodyparts    
        ...
        switch DLCiteration
            case 'iteration-1'   
                file_trace=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,'iteration1',filesep,dataSummaryT.DLCFileName1{iSession},'.csv'];
                if ~exist(file_trace,'file')%when no multiple files, just place the file at root path
                    file_trace=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,dataSummaryT.DLCFileName1{iSession},'.csv'];
                end
                flagSetNAN=0;
            case 'iteration-2'
                file_trace=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,'iteration2',filesep,dataSummaryT.DLCFileName2{iSession},'.csv'];
                flagSetNAN=0;
            case 'iteration-3'
                file_trace=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,'iteration3',filesep,dataSummaryT.DLCFileName3{iSession},'.csv'];
                flagSetNAN=0;
            case 'iteration-4'
                file_trace=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,'iteration4',filesep,dataSummaryT.DLCFileName4{iSession},'.csv'];
                flagSetNAN=0;
            case 'iteration-5'
                file_trace=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,'iteration5',filesep,dataSummaryT.DLCFileName5{iSession},'.csv'];
                flagSetNAN=0;
            otherwise %no data, skip this loop
                file_trace=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,dataSummaryT.DLCFileName1{iSession},'.csv'];%for  iteration-1~3
%                 file_trace=[dataSummaryT.rootpath{iSession},filesep,'video',filesep,'iteration4',filesep,dataSummaryT.DLCFileName4{iSession},'.csv'];%for  iteration-4+
...
%         indSig=(pTtest<pSigTtest);
        ...
        figure(figMeanDiff);
        subplot(size(bodyparts,2),size(bodyparts,1),iBodyPart);
        temp3=fInter1(matMeanDiffEachSession(:,iBodyPart));
        matCasesStimResult=cell2mat(temp3);
