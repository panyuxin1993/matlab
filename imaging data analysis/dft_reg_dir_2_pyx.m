function dft_reg_dir_2_pyx(src_dir, save_path, main_fname, targetImage, varargin)
...
% - NX 2013-06-12
%
% Update for 2 channels simultaneously recording
% varargin{3}, number of channels, default,1
...
datafiles = dir([src_dir filesep main_fname '.tif']);%datafiles = dir([src_dir filesep main_fname '*.tif']); %error:Characters adjacent to a ** wildcard must be file separators.
...
if default_target == 1
    ...
    [im_temp, ~] = load_scim_data_pyx([src_dir filesep datafiles(n).name], [], 1,nchannel);
...
%%%%% READ SOURCE IMAGE DATA. With the option of offset data to mode.
    [im_s, header] = load_scim_data_pyx(fname, [], 1,nchannel);
    imTagStruct = get_tiff_tag_to_struct(fname);
    if nchannel~=1 %if make substack, then the tiff tag should also be subsampled
        imTagStruct=imTagStruct(1:nchannel:end);
    end
%     K = wiener2(J,[5 5]);
