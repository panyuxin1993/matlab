...
clear;
close all;
sessionNamePool={'pyx232_20200906','pyx209_20190613'};
behEventAlignPool={'delay onset','first lick'};
masklickPool={'yes','no'};
...
for roiNo = 1:size(SavedCaTrials.f_raw{1},1) %SavedCaTrials.nROIs may be not true
   ...
   if strcmp(behEventAlign,'delay onset') && strcmp(masklick,'yes')%by default, if align to stim onset, then see delay activity and mask all after go cue as nan
    ...
    set(gcf, 'position', [0 100 1400 300]);
...
% set(A,'PaperPosition',[0,0,8,4]);  
% saveas(A,['F:\2P\example\',sessionName,'ROI-',num2str(roiNo),'-algin to ',behEventAlign,'-sort ',behEventSort,'colorplot.pdf'],'pdf');
% set(figMeanTrace,'PaperPosition',[0,0,8,2]);     
% saveas(figMeanTrace,['F:\2P\example\',sessionName,'ROI-',num2str(roiNo),'-algin to ',behEventAlign,'-sort ',behEventSort,'mean trace.pdf'],'pdf');
    close all;
