...
for i_session=1:n_session
    ...
    T_currentSession=fGetCorrelationRsc(T.file_path{indrow},T.animal{indrow},T.date{indrow},T.field{indrow},T.cell_type{indrow},trialTypeStr,selectivityType);
    if exist('TcorrelationRT','var')
        Trsc=vertcat(Trsc,T_currentSession);
    else
        Trsc=T_currentSession;
    end
end
save([savepath,filesep,'trialType',trialTypeStr,'-selectivity',selectivityType,'Rsc.mat'],'Trsc');
%}
load([savepath,filesep,'trialType',trialTypeStr,'-selectivity',selectivityType,'Rsc.mat']);
...
TcorrRT=Trsc(strcmp(Trsc.celltype,celltype),:);
