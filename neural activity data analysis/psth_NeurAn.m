...
if plt == 'n';return;end
% fill([t t],[R-2*E R+2*E],[0 0.1 0]);  
hold on
plot(t,R,plt,'LineWidth',3);
box off
% hold on
if err > 0
  plot(t,R+2*E,'g');
...
% axis([-1.5 1 0 80])
% axis([-1.5 1 0 1.5*max(R)])
% xlabel(['Time Align to Move Onset (s)'],'FontSize',14,'FontWeight','bold')
% ylabel('Firing Rate (Hz)','FontSize',14,'FontWeight','bold')
% set(gca,'XTick',-2:1:2,'FontSize',24,'FontWeight','bold');
% set(gca,'YTick',0:20:2*max(R),'FontSize',24,'FontWeight','bold');
R_max=max(R);
