function [TAUC, Tmean] = fGetEpochAUCtableASession(filepath,animal,date,field,celltype,trialTypeStr,AUCtype,varargin)
...
if false%~exist('Tmean','var')
    ...
    for roiNo = 1:size(SavedCaTrials.f_raw{1},1) %SavedCaTrials.nROIs may be not true
        [T_SigbyEpoch,str_nFrames] = fGetSigBehEpoch(behEventFrameIndex,dff(roiNo,:),frT,str_nFrames);
        T_SigbyEpoch=T_SigbyEpoch(ind_trial,:);
        [ITI_m(roiNo),pITI_m(roiNo)]=fMeanPdiff(T_SigbyEpoch.ITI,T_SigbyEpoch.ITI);
        [sound_m(roiNo), psound_m(roiNo)]=fMeanPdiff(T_SigbyEpoch.ITI,T_SigbyEpoch.sound);
        [delay_m(roiNo),pdelay_m(roiNo)]=fMeanPdiff(T_SigbyEpoch.ITI,T_SigbyEpoch.delay);
        [response_m(roiNo),presponse_m(roiNo)]=fMeanPdiff(T_SigbyEpoch.ITI,T_SigbyEpoch.response);
        [lick_m(roiNo),plick_m(roiNo)]=fMeanPdiff(T_SigbyEpoch.ITI,T_SigbyEpoch.lick);
    end
    [varanimal{1:nROI}]=deal(animal);
    varanimal=reshape(varanimal,[],1);
    [vardate{1:nROI}]=deal(date);
